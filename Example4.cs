...
{
    public abstract class Bird //Открытость/закрытость Код не позволяет легко добавлять новые виды птиц без изменения базового класса. Чтобы добавить новую птицу которая не может летать придется изменять метод fly
    {
